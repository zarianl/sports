...
import { PrismaClient, type Team, type Game } from '@prisma/client';
            ...
            >
                ...
                >
                        ...
                        <Grid item xs={12}>
                            <FormControl fullWidth>
                                <TextField
                                    ...
                                    value={line}
                                    onChange={(event: React.ChangeEvent<{ value: unknown }>) => setLine(event.target.value as number)}
                                />
                            </FormControl>
                        </Grid>
                    </Grid>
                    ...
                    <Typography variant="h6">
                        Home Average Score: {
                            Math.round((
                                getAverageFirstHalfScore(teams.find(t => t.id === Number(homeTeam)) ?? defaultTeam, 'home', 'scores') +
                                getAverageFirstHalfScore(teams.find(t => t.id === Number(awayTeam)) ?? defaultTeam, 'away', 'allow')
                            ) / 2 * 10) / 10
                        }
                    </Typography>
                    ...
                    <Typography variant="h6">
                        Away Average Score: {
                            Math.round((
                                getAverageFirstHalfScore(teams.find(t => t.id === Number(homeTeam)) ?? defaultTeam, 'away', 'scores') +
                                getAverageFirstHalfScore(teams.find(t => t.id === Number(awayTeam)) ?? defaultTeam, 'home', 'allow')
                            ) / 2 * 10) / 10
                        }
                    </Typography>
                    <Typography variant="h6">
                        Total Average Score: {
                            Math.round((
                                (getAverageFirstHalfScore(teams.find(t => t.id === Number(homeTeam)) ?? defaultTeam, 'home', 'scores') +
                                    getAverageFirstHalfScore(teams.find(t => t.id === Number(awayTeam)) ?? defaultTeam, 'away', 'allow')) / 2 +
                                (getAverageFirstHalfScore(teams.find(t => t.id === Number(homeTeam)) ?? defaultTeam, 'away', 'scores') +
                                    getAverageFirstHalfScore(teams.find(t => t.id === Number(awayTeam)) ?? defaultTeam, 'home', 'allow')) / 2) * 10) / 10
                        }
                    </Typography>
                    ...
                    <TableBody>
                        {teams.map((team) => (
                            <TableRow key={team.id}>
                                ...
                                <TableCell>{Math.round(getAverageFirstHalfScore(team, 'home', 'scores') * 10) / 10}</TableCell>
                                <TableCell>{Math.round(getAverageFirstHalfScore(team, 'away', 'scores') * 10) / 10}</TableCell>
                                <TableCell>{Math.round(getAverageFirstHalfScore(team, 'home', 'allow') * 10) / 10}</TableCell>
                                <TableCell>{Math.round(getAverageFirstHalfScore(team, 'away', 'allow') * 10) / 10}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
